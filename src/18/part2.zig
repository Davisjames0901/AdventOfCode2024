...
pub fn main() !void {
    ...
    while(lines.peek()) |_| : (i += 1) {
        unvisited.clearRetainingCapacity();
        visited.clearRetainingCapacity();

        const newest = try Vec2.init(lines.next().?, lines.next().?);
        try bytes.put(newest, {});
        ...
        if(visited.contains(mapSize)) {
            std.debug.print("{} is routable\n", .{i});
        } else {
            std.debug.print("{} is not routable\n{any}", .{i, newest});
            break;
        }
    }
}
